...
def load_tomogram_worker(tomogram, resolution_level=0):
    ...
    try:
        logger.debug(f"Starting tomogram loading for {tomogram.meta.tomo_type} at resolution level {resolution_level}")

        zarr_path = tomogram.zarr()
        logger.debug(f"Zarr path: {zarr_path}")

        yield f"Opening zarr group for {tomogram.meta.tomo_type}..."
        zarr_group = zarr.open(zarr_path, "r")
        logger.debug(f"Zarr group opened successfully. Keys: {list(zarr_group.keys())}")
        ...
        logger.debug(f"Found {len(scale_levels)} scale levels: {scale_levels}")
        ...
        if resolution_level >= len(scale_levels):
            logger.warning(
                f"Requested resolution level {resolution_level} not available, using highest available: {len(scale_levels)-1}",
            )
            resolution_level = len(scale_levels) - 1

        # Load only the selected resolution level
        selected_level = scale_levels[resolution_level]
        logger.debug(f"Loading resolution level {resolution_level} (scale level {selected_level})")

        yield f"Loading resolution level {resolution_level}..."
        array = zarr_group[selected_level]
        logger.debug(f"Array shape: {array.shape}, chunks: {array.chunks}")
        ...
        logger.debug(f"Voxel size for level {resolution_level}: {voxel_size}")

        # Actually load the data (not lazy!)
        yield "Loading image data into memory..."
        logger.debug("Loading array data into memory...")
        loaded_data = np.array(array)
        logger.debug(f"Data loaded successfully. Final shape: {loaded_data.shape}")
...
def load_segmentation_worker(segmentation, resolution_level=0):
    ...
    try:
        logger.debug(
            f"Starting segmentation loading for {segmentation.meta.name} at resolution level {resolution_level}",
        )
        ...
        if "data" in zarr_group:
            ...
            logger.debug(f"Using multiscale level {resolution_level} (key: {data_key})")
        else:
            # Fallback to first available key
            data_key = list(zarr_group.keys())[0]
            logger.debug(f"Using fallback data key: {data_key}")

        yield f"Loading segmentation data from level {resolution_level}..."
        array = zarr_group[data_key]
        logger.debug(f"Array shape: {array.shape}, chunks: {getattr(array, 'chunks', 'N/A')}")
...
def expand_run_worker(run):
    ...
    try:
        ...
        logger.debug("Run expansion completed successfully")
...
def expand_voxel_spacing_worker(voxel_spacing):
    ...
    try:
        logger.debug(f"Starting voxel spacing expansion for {voxel_spacing.meta.voxel_size}")
